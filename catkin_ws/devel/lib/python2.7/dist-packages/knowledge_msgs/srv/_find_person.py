...
class find_personResponse(genpy.Message):
  _md5sum = "676f485aabec4ad84af601202706a726"
...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      _x = self
      buff.write(_get_struct_B2bB().pack(_x.person_face.smile, _x.person_face.gender, _x.person_face.ages, _x.task_success))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      (_x.person_face.smile, _x.person_face.gender, _x.person_face.ages, _x.task_success,) = _get_struct_B2bB().unpack(str[start:end])
...
class find_person(object):
  ...
  _md5sum = '4cbf46b2d7756b2ab780c337bd6e1574'
