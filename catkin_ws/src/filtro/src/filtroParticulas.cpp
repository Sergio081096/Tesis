...
void send_odom_map_transform(tf::TransformBroadcaster& tb, float odom_x, float odom_y, float odom_t,
                             std::vector<geometry_msgs::Pose>& particles)
{
    float mean_x = 0;
    float mean_y = 0;
    ...
    for(size_t i=0; i < particles.size(); i++)
    {
        mean_x += particles[i].position.x;
        mean_y += particles[i].position.y;
        mean_t += atan2(particles[i].orientation.z, particles[i].orientation.w)*2;
    }
    mean_x /= particles.size();
    mean_y /= particles.size();
    mean_t /= particles.size();
}

int main(int argc, char** argv)
{
    //std::cout << "filtroParticulas - " << NOMBRE << std::endl;
    ros::init(argc, argv, "filtroParticulas");
    ros::NodeHandle n("~");
    ros::Rate loop(10);
    ...
    ros::service::call("/navigation/static_map", srv_get_map);
