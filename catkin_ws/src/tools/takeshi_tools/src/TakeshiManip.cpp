...
bool TakeshiManip::moveitGoalPosition(std::string move_group, float x, float y, float z, float alpha, float beta, float gamma){
  
  geometry_msgs::Pose pose;

  tf2::Quaternion myQuaternion;
  myQuaternion.setRPY( alpha,beta, gamma);
  pose.position.x=x;
  pose.position.y=y;
  pose.position.z=z;
